...
def genSimplePassword():
    ...
    while True:
        ...
        if (sum(c.islower() for c in password) == 3
                and sum(c.isupper()
                        for c in password) == 3 and sum(c.isdigit()
                                                        for c in password) == 3
                and sum(assertStr(s) for s in password) == 2
                and password[0] != '_' and password[-1] != '_'
                and password[0] != '#' and password[-1] != '#'
                and '__' not in password and '##' not in password
                and '_#' not in password and '#_' not in password):
            break
    print(password)
